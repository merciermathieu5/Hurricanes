...
(async () => {
  ...
  egal(S.SECOURS_ROSTER.length, 23, '23 joueurs dans la formation de secours (pro, aucun backup)');
  const drou = S.SECOURS_ROSTER.find(j => j.nom === 'Jonathan Drouin');
  egal(drou.salaire, 8500000, 'Salaire Drouin');
  egal(drou.ct, 1, 'Contrat Drouin 1 an');
  egal(S.SECOURS_Y21.length, 20, '20 patineurs dans la référence Y21 (XtraStats, validés par ratios)');
  const drouY21 = S.SECOURS_Y21.find(x => x.nom === 'Jonathan Drouin');
  egal(drouY21.pts, 88, 'Drouin 88 points en Y21');
  egal(drouY21.mp, 2144, 'Drouin 2144 minutes en Y21 (26 min/match!)');
  const debY21 = S.SECOURS_Y21.find(x => x.nom === 'Jake DeBrusk');
  egal(debY21.goals, 30, 'DeBrusk 30 buts en Y21');
  egal(debY21.hits, 258, 'DeBrusk 258 MEÉ en Y21');
  const chiaY21 = S.SECOURS_Y21.find(x => x.nom === 'Jake Chiasson');
  egal(chiaY21.gp, 10, 'Chiasson : dix matchs en Y21 (transcrit tel quel)');
  ...
  egal(profilDe('Jonathan Huberdeau').profil, 'Elite', 'Huberdeau → Elite');
  tableauEgal(profilDe('Jonathan Huberdeau').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Jonathan Drouin').profil, 'Playmaker', 'Drouin → Playmaker');
  tableauEgal(profilDe('Jonathan Drouin').stats, ['assists','pts'], 'Stats Playmaker = A, P');
  egal(profilDe('Jake DeBrusk').profil, 'Power Forward', 'DeBrusk → Power Forward');
  egal(profilDe('Jeff Skinner').profil, 'Sniper', 'Skinner → Sniper');
  egal(profilDe('William Carrier').profil, 'Grinder', 'Carrier → Grinder');
  tableauEgal(profilDe('William Carrier').stats, ['hits20'], 'Stats Grinder = MEÉ/20');
  egal(profilDe('Jonatan Berggren').profil, 'Depth Forward', 'Berggren → attaquant de profondeur');
  egal(profilDe('Fabian Zetterlund').profil, 'Two-Way Forward', 'Zetterlund → Two-Way Forward');
  egal(profilDe('Shane Wright').profil, 'Prospect Elite', 'Wright (21 ans) → Prospect Elite');
  egal(profilDe('Shane Wright').mat, 'ELITE', 'Prospect Elite → même matrice ELITE');
  egal(profilDe('Tristan Broz').profil, 'Prospect Two-Way Forward', 'Broz (23 ans) → Prospect Two-Way');
  egal(profilDe('Rasmus Dahlin').profil, 'DEliteQB', 'Dahlin → DEliteQB');
  egal(profilDe('Hampus Lindholm').profil, 'DEliteShutdown', 'Lindholm → DEliteShutdown');
  egal(profilDe('Jared Spurgeon').profil, 'DOffensive', 'Spurgeon → DOffensive');
  egal(profilDe('Erik Cernak').profil, 'DDefensive', 'Cernak → DDefensive');
  egal(profilDe('Layton Ahac').profil, 'Prospect DEliteShutdown', 'Ahac (24 ans) → Prospect DEliteShutdown');
  egal(profilDe('Pyotr Kochetkov').profil, 'Starter Goalie', 'Kochetkov (OV 80) → Starter Goalie');
  tableauEgal(profilDe('Pyotr Kochetkov').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Yaroslav Askarov').profil, 'Starter Goalie', 'Askarov (OV 80, 23 ans) → Starter Goalie (règle 6.2.5, peu importe l\'âge)');
  egal(profilDe('Kaapo Kahkonen').profil, 'Backup Goalie', 'Kahkonen (OV 77) → Backup Goalie');
  tableauEgal(profilDe('Kaapo Kahkonen').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const four = S.fourchetteRecote(drou, 10); // 30 ans → ModA-5 → base 65
  ...
  const jk = joueurDe('Jonathan Drouin');
  jk._profil = S.determinerProfil(jk);
  let s = S.seuilsPour(jk, 'pts', {}, {});
  tableauEgal(s.seuils, [88,78,67,57,34,-1], 'Seuils Y17 par défaut (OV 83)');
  egal(s.source, 'Y17', 'Source = Y17');
  const perso = {PLAYMAKER: {pts: {83: [120,110,100,90,50,-1]}}};
  ...
  egal(s.seuils[2], 67, 'Les autres degrés restent ceux de la matrice');
  ...
  const fixtureXtra = [
    ...
    '    [Jonathan Drouin](https://x/#Jonathan Drouin) CAROLINA        C   82  30  45  75 200  30 1700 100 2.44',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) CAROLINA        LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'CAROLINA');
  egal(x.length, 2, 'Deux patineurs CAROLINA extraits (Anaheim exclu)');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('CAROLINA'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 79435300, 'Masse salariale des 23 pros = 79 435 300 $');
  ...
  console.log('— Divers');
  egal(S.matchsEquipe(), 0, 'Fiche CAROLINA 0-0-0 → 0 match d\'équipe');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Jonathan Drouin', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jDrou = S.SECOURS_ROSTER.find(x2=>x2.nom==='Jonathan Drouin');
    const prodRepli = S.productionDe(jDrou);
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Jonathan Drouin'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 88, 'Points Y21 de Drouin');
  const carteDrou = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Jonathan Drouin');
  ok(!!carteDrou, 'Carte de Drouin rendue en mode Y21');
  ok(carteDrou.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jDr = S.ETAT.roster.find(x2=>x2.nom==='Jonathan Drouin');
  const evPtsD = S.evaluerStat('pts', S.valeurStat(jDr, prodY21, 'pts', {}), S.seuilsPour(jDr, 'pts', {}, {}).seuils);
  egal(evPtsD.statut, 'excellente',
    'Drouin Excellente en points : 88 = seuil Mémorable exactement, or le tableau 18 exige un dépassement STRICT');
  const jDb = S.ETAT.roster.find(x2=>x2.nom==='Jake DeBrusk');
  const evButsB = S.evaluerStat('goals', S.valeurStat(jDb, S.productionDe(jDb), 'goals', {}), S.seuilsPour(jDb, 'goals', {}, {}).seuils);
  egal(evButsB.statut, 'satisfaisante', 'DeBrusk Satisfaisante en buts (28 < 30 ≤ 33, Power Forward OV 84)');
  ok(!carteDrou.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
